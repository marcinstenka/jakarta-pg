...
Once the runtime starts, you can access the project at [http://localhost:8080/heroes](http://localhost:8080/heroes).
...
```
./mvnw clean package
docker build -t heroes:v1 .
```

You can then run the Docker image by executing:

```
docker run -it --rm -p 8080:8080 heroes:v1
```
